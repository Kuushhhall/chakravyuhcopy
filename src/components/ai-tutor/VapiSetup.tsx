...
interface VapiSetupProps {
  onSetupComplete: (apiKey: string) => void;
}

export function VapiSetup({ onSetupComplete }: VapiSetupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!apiKey.trim()) {
      toast({
        ...
        description: "Please enter your Vapi AI API key",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-8">
      <Card>
        <CardHeader>
          <CardTitle className="text-center">Vapi AI Setup</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <p className="mb-4 text-sm text-muted-foreground">
                To enable voice conversation, please enter your Vapi AI API key.
                You can find or create your API key in the{" "}
                <a 
                  href="https://app.vapi.ai/settings/api-keys"
                  target="_blank"
        ...
        <CardFooter className="flex justify-center text-xs text-muted-foreground">
          <p>
            Don't have a Vapi AI account?{" "}
            <a 
              href="https://app.vapi.ai/signup"
              target="_blank"
              rel="noopener noreferrer"
              className="text-primary underline"
            >
